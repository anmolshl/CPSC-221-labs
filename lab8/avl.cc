...
void balance( Node *& x ) {
    if (std::abs(get_balance(x)) <= 1) {            //balanced
        return;
    }
    else {
        if (get_balance(x) > 1) {                   //more left
            if (get_balance(x->left) >= 0)
                rotateRight(x);                     //more left-left
            else
                doubleRotateRight(x);               //balanced after execution
        }
        else if (get_balance(x) < 1) {              //more right
            if (get_balance(x->right) <= 0)
                rotateLeft(x);                      //more right-right
            else
                doubleRotateLeft(x);                //balanced after execution
        }
    }
}
